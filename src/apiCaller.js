...
class APICaller {
  ...
  constructor(config) {
    if (!config) {
      config = new EvtConfig();
    } else if (
      typeof config == "object" &&
      config != null &&
      !(config instanceof EvtConfig)
    ) {
  ...
  async getNodeTimestamp() {
    if (this.timeDiff == null && this.config.syncNodeTime) {
      try {
  ...
  async getInfo(options) {
    ...
    if (
      !info.evt_api_version.startsWith("2.") &&
      !info.evt_api_version.startsWith("3.") &&
      !info.evt_api_version.startsWith("4.")
    ) {
      throw new Error(
        `[Fatal] The API version of remote net (${info.evt_api_version}) is not compatible with current evtjs's version. Please upgrade your evtjs's version.`
      );
    }
    ...
    if (
      ...
      !info.enabled_plugins.includes("evt::history_plugin")
    ) {
    ...
    this.timeDiff =
      new Date(info.head_block_time).getTime() + 70 - new Date().getTime();
  ...
  async getTransactionDetailById(
    ...
    let res = await this.__callAPI({
      url:
        _options.usingHistoryPlugin == false
          ? "/v1/chain/get_transaction"
  ...
  async getBlockDetail(blockNumOrId) {
    if (typeof blockNumOrId !== "string" || !blockNumOrId)
      throw new Error(
        "invalid query key, it can be either block_num or block_id"
      );
  ...
  async getTransactionIdForLinkId(id) {
    console.warn(
      "[warn] getTransactionIdForLinkId is deprecated, please use getStatusForLinkId instead."
    );
  ...
  async getStatusOfEvtLink(options) {
    ...
    if (lastTimeInternet) {
      return {
        pending: false,
        successful: false,
        exception: lastError || new Error("everiPay timeOut or network issue")
      };
    } else {
      return {
        pending: true,
        successful: false,
        exception: new Error("No available network connection")
      };
    }
  }
  ...
  async getEstimatedChargeForTransaction() {
    ...
    // Get required keys
    Logger.verbose(
      "[getEstimatedChargeForTransaction] get required_keys, available keys: " +
        JSON.stringify(p.publicKeys, null, 4)
    );
    let apiRes = await this.__chainGetRequiredKeys({
      transaction: p.body.transaction,
      available_keys: p.publicKeys
    });
    let required_keys = apiRes.required_keys;

    Logger.verbose(
      "[getEstimatedChargeForTransaction] got required_keys: " +
        JSON.stringify(required_keys, null, 4)
    );
  ...
  async getTransactionIdsInBlock(blockId) {
    ...
    if (blockId.length < 64)
      throw new Error(
        "invalid blockId. Note: block id is not the same as block num."
      );
  ...
  async getFungibleBalance(address, symbolId) {
    ...
    if (symbolId) {
      ...
      if (isNewerVersion && !this.__node_history_plugin_enabled) {
        throw new Error(
          "calling getFungibleBalance without symbolId parameter is only supported when history plugin is eneabled on the connected node if evt api version >= 4.0"
        );
      }
    }
    ...
    let args = {
      url:
        isNewerVersion && !symbolId
          ? "/v1/history/get_fungibles_balance"
          : "/v1/evt/get_fungible_balance",
      method: "POST",
      body,
      sign: false // no need to sign
    };
   ...
   */
  async getFungibleActionsByAddress(
    symbolId,
    address = undefined,
    skip = 0,
    take = 10
   ...
   */
  async getTransactionsDetailOfPublicKeys(
    publicKeys,
    skip = 0,
    take = 10,
    direction = "desc"
    ...
    if (direction !== "desc" && direction !== "asc") {
      throw new Error(
        "parameter `direction` could only be one of `desc` or `asc`, but `" +
          direction +
          "` given"
      );
    }
  ...
  __throwServerResponseError(res) {
    ...
    let err = new Error(
      `[${res.code} ${res.message}] (${(res.error || {}).code}) ${
        (res.error || {}).name
      }: ${(res.error || {}).what} ${details}`
    );
    ...
    Logger.verbose(
      "[__throwServerResponseError] node's response return error:\n" +
  ...
  async pushTransaction() {
    ...
    if (
      arguments.length > 0 &&
      !(arguments[0] instanceof EvtAction) &&
      !arguments[0].action
    ) {
      // config found
    ...
    if (!trxConf.__estimateCharge || !trxConf.availablePublicKeys) {
      ...
      publicKeys = await this.__calcPublicKeyProvider(
        trxConf.availablePublicKeys
      );
    }
    ...
    if (trxConf.maxCharge == null || !Number.isInteger(trxConf.maxCharge))
      throw new Error(
        "maxCharge is required and must be a integer greater than or eqaul to 0"
      );
    if (
      !trxConf.__estimateCharge &&
      (trxConf.payer == null || !EvtKey.isValidAddress(trxConf.payer))
    )
      throw new Error(
        "payer is required and must be a valid address as a string"
      );
    ...
    for (let i = 0; i < body.transaction.actions.length; ++i) {
      if (!(body.transaction.actions[i] instanceof EvtAction)) {
        body.transaction.actions[i] = new EvtAction(
          body.transaction.actions[i].action,
          body.transaction.actions[i].args
        );
      }
      ...
      let binAction = {
        ...
        data: (
          await this.__chainAbiJsonToBin(
            { action: originalAction.actionName, args: originalAction.abi },
            trxConf.AbiJsonToBinLocally
          )
        ).binargs,
      ...
      if (binAction.name == "everipay") {
        ...
        trxConf.__linkId = (
          await EvtLink.parseEvtLink(originalAction.abi.link)
        ).segments
          .filter(x => x.typeKey == 156)[0]
          .value.toString("hex");

        if (trxConf.expiration) {
          throw new Error(
            "Expiration can not be set in a transaction including a everipay action, the expiration must be set automatically by evtjs"
          );
        }
      }
    ...
    if (trxConf.expiration) {
      ...
      if (trxConf.__hasEveriPay) {
        ...
        expiration = new Date((await this.getNodeTimestamp()) + 10000)
          .toISOString()
          .substr(0, 19);
      } else {
        expiration = new Date((await this.getNodeTimestamp()) + 100000)
          .toISOString()
          .substr(0, 19);
      }
    }

    hash = ByteBuffer.fromHex(
      this.__cachedInfo.last_irreversible_block_id,
      true
    ); // little endian
    numHex = this.__cachedInfo.last_irreversible_block_id.substr(4, 4);
    last_irreversible_block_num = ByteBuffer.fromHex(numHex, false).readUint16(
      0
    );
    ...
    if (params.sign) {
      ...
      let sigs = await this.__signTransaction(
        signBuf,
        body.transaction,
        privateKeys,
        rawActions
    ...
    return await this.pushSignedTransaction(
      body,
      trxConf.__hasEveriPay ? trxConf.__linkId : null
    );
  }
  ...
  async pushSignedTransaction(body, includedEvtLinkID = null) {
    ...
    if (includedEvtLinkID !== null) {
      ...
      let ret = await this.getStatusOfEvtLink({
        linkId: includedEvtLinkID,
        block: true,
        throwException: false
      });
      ...
      res &&
      res.processed &&
      res.processed.receipt &&
      res.processed.receipt.status === "executed"
    ) {
      return {
        transactionId: res.transaction_id,
        charge: res.processed.charge,
        body
      };
    } else {
      // throw error detail
  ...
  __signTransaction(buf, transaction, privateKeys, actions) {
    return this.config.signProvider({
      signHash,
      buf,
      transaction,
      privateKeys,
...
*/
const defaultSignProvider = (apiCaller, config) =>
  async function({ signHash, buf, transaction, privateKeys }) {
    let keys = privateKeys;

    if (!keys) {
      throw new TypeError(
        "This transaction requires keyProvider for signing, please check your initial parameter of APICaller class."
      );
    }
    ...
    Logger.verbose(
      "[defaultSignProvider] get required_keys, available keys: " +
        JSON.stringify(
          keys.map(key => key.public),
          null,
          4
        )
    );
    let apiRes = await apiCaller.__chainGetRequiredKeys({
      transaction,
      available_keys: keys.map(key => key.public)
    });
    let required_keys = apiRes.required_keys;

    Logger.verbose(
      "[defaultSignProvider] got required_keys: " +
        JSON.stringify(apiRes, null, 4)
    );
